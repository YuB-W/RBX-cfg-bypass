...
// Hyprion Version: version-38d71f95cf424504
...
#define CFG_PAGE_HASH_KEY       0x7244ee56
#define CFG_VALIDATION_XOR      0xe8

#define HashPage(Page) \
    (((int32_t)((uintptr_t)(Page) >> 12)) ^ CFG_PAGE_HASH_KEY)

#define ValidationByte(Page) \
    (((int8_t)((uintptr_t)(Page) >> 44)) ^ CFG_VALIDATION_XOR)
...
{                                                                                                       \
    ...
    {                                                                                                   \
        int32_t page_hash = HashPage(Page);                                                             \
        uint8_t validation = ValidationByte(Page);                                                      \
                                                                                                        \
        *reinterpret_cast<int32_t*>(stack_block + 0x18) = page_hash;                                    \
        *reinterpret_cast<uint8_t*>(stack_block + 0x1C) = validation;                                   \
                                                                                                        \
        insert_set(whitelist, stack_block + 0x28, stack_block + 0x18);                                  \
    }                                                                                                   \
}
